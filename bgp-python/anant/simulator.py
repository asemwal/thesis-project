...
#		AS_graph = self.SI.add_forwarding_rule( AS_graph = AS_graph, AS_number = 1, from_type = self.SI.get_P2P_type(), to_type = self.SI.get_C2P_type(), allow = True )
	...
	def insert_announcements_demo2( self , filestamp = '' ):
		self.P.write( "insert_announcements_demo: start", color = 'green' )
		a =1
		if a != 1:
			self.P.write(a)
		...
		while l !='':
			#self.P.write(l)
			x = l.split("|")
			#self.P.write("printing x after split"+ str(x))

			l = str(file.readline()).strip()
			#self.P.write(l)
			if str(x[2]) == 'peer-to-peer':
				#self.P.write('p2p')
				AS_graph = self.SI.add_peer( AS_graph = AS_graph, AS_number = int(x[0]), peer = int(x[1]) )
			elif str(x[2]) == 'provider-to-customer':
				#self.P.write('p2c')
				AS_graph = self.SI.add_customer( AS_graph = AS_graph, AS_number = int(x[0]), customer = int(x[1]) )
			elif str(x[2]) == 'customer-to-provider':
				#self.P.write('c2p')
				AS_graph = self.SI.add_customer( AS_graph = AS_graph, AS_number = int(x[1]), customer = int(x[0]) )
			elif str(x[2]) == 'sibling-to-sibling':
				#self.P.write('s2s')
				AS_graph = self.SI.add_sibling( AS_graph = AS_graph, AS_number = int(x[0]), sibling = int(x[1]) )
		working =working
		if str(working) not in AS_graph.keys():
			self.P.write("Victim not in graph. Returning for "+ str(working))
			return
		#exit()
...
#		AS_graph = self.SI.add_forwarding_rule( AS_graph = AS_graph, AS_number = 1, from_type = self.SI.get_P2P_type(), to_type = self.SI.get_C2P_type(), allow = True )
		...
		#AS_Graph = self.SI.load_AS_graph( file_name = "insert_announcements_demo", print_status = True )
		# Create and Add GOOD route to AS_graph
		announcement = self.SI.create_announcement( prefix = "192.168.0.0/24", AS_number = working, good = True )
		AS_graph = self.SI.insert_announcement( AS_graph = AS_graph, announcement = announcement )
		...
		for i in peerlist:
			if str(i) not in process:
				self.P.write(str(i))

		process.remove(str(working))


		self.P.write(len(process))
		for i in process:
			AS_graph = self.SI.load_AS_graph( file_name = "insert_announcements_demo", print_status = True )
			self.P.write("Number of Nodes:"+ str(len(AS_graph.keys()) ))
			# Create and Add GOOD route to AS_graph
			announcement = self.SI.create_announcement( prefix = "192.168.0.0/24", AS_number = i, good = False )
			AS_graph = self.SI.insert_announcement( AS_graph = AS_graph, announcement = announcement )
		...
		# Save / Load Routing Table
			...
			for peer in peerlist:
				...
				bad = False
				code = 'N'
				for j in range(0, len(routing_table[str(peer)]['192.168.0.0/24'])):
					if routing_table[str(peer)]['192.168.0.0/24'][j]['used'] == True :
						used0 = True
						if routing_table[str(peer)]['192.168.0.0/24'][0]['good'] == True:
							good = True
						else:
							bad = True
				...
				if peer in routing_table.keys():
					h = h+"|"+ str(peer)
					#l = l +str(len(routing_table[str(peer)]['192.168.0.0/24']))+"|"
					l = l+"|" +str(code)
				else:
					h = h+"|"+ str(peer)
					l = l+"|"+ "KeyError" 
			file = open('/home/asemwal/thesis/bgp-python_safe/data/simulator/myrun/'+str(working)+'.txt','a')
			op = h + "\n" +l +"\n"
			file.write(op)
			file.flush()
			file.close()
			self.P.write(AS_graph['1'])
	...
	def run( self ):
		...
		if mode == "1":
			...
			for f in onlyfiles:
				...
				self.insert_announcements_demo(graphname = f , pfxfile = 'prefixassoc_'+timestamp)
				os.rename(mypath+f , mypath+"done/"+f)
				os.rename(mypath+'prefixassoc_'+timestamp , mypath+"done/"+'prefixassoc_'+timestamp)
