...
import numpy as np
import time
#import  stg4/monitorselection as ms
...
def toygraphgenerator(graphsize = 100):
    ...
    addprovider(list(tier_1), list(tier_1) , list_of_provider= list(range(0,len(tier_1)-1)), k  = -1 , relationship= 'peer-to-peer')
...
def testgraohgeneration(n = 1, size = 63000 ):    
    dir = '/home/asemwal/raw_data/experiments/graphs/'     
    for i in range(0,n):
        timestamp = toygraphgenerator2(size)
        #ms.monitorselection(dir+ "graph_"+timestamp , timestamp)
        time.sleep(1)
        

def toygraphgenerator2(graphsize = 100):
    global g
    if graphsize < 100:
        return
    g = graph(graphsize)
    ...
    #prefixowners.update(prefixowners.union(set(tier_4 )))
    #prefixassoc = prefixassociation(list(prefixowners) , [1 , 2] , k = 1)
    addpeerT1(list(tier_1))
    addnode(list(tier_2), tier_1)
    addnode(list(tier_3), tier_2)
    #addnode(list(tier_4), tier_3)
    #addprovider(list(tier_4), list(tier_3) , list_of_provider= [1], k  = 5 , relationship= 'provider-to-customer')
    #addprovider(list(tier_4), list(tier_2) , list_of_provider= [1 ], k  = 4 , relationship= 'provider-to-customer')
    #addprovider(list(tier_3), list(tier_2) , list_of_provider= [1], k  = 2 , relationship= 'provider-to-customer')
    #addprovider(list(tier_3), list(tier_1) , list_of_provider= [1 ], k  = 4 , relationship= 'provider-to-customer')
    #addprovider(list(tier_2), list(tier_1) , list_of_provider= [1], k  = 1 , relationship= 'provider-to-customer')


    print(g.number_of_edges())
    print(len(tier_3))
    #addprovider(list(tier_3), list(tier_3) , list_of_provider=  [1], k  = 3 , relationship= 'peer-to-peer')
    #addprovider(list(tier_2), list(tier_2) , list_of_provider=  [1], k  = -1 , relationship= 'peer-to-peer')
    #addprovider(list(tier_1), list(tier_1) , list_of_provider= [1], k  = -1 , relationship= 'peer-to-peer')
...
def addpeerT1(list1 = [] , r = 'p2p'):
    global g
    while len(list1) > 0 :
        x= list1.pop()
        for j in list1:
            g.add_edge(x , j, relationship = r)
            g.add_edge(j , x, relationship = r)
...
def addnode(list1 = [], list2=[] , r = 'p2c'):
    ...
    while len(list1) > 0 :
        ...
        for i in range(0, len(list2)):
            prob.append(p[list2[i]])
            P+=prob[-1]
        for i in range(0, len(list2)):
            prob[i]/=P
        provider = list(np.random.choice(list2, 1, p=prob, replace = False))
        g.add_edge(x , provider[0] , relationship = 'c2p')
        g.add_edge( provider[0] ,x, relationship = 'p2c')
