...
#    c = {0: 'peer-to-peer', -1: 'provider-to-customer'}
    g = nx.DiGraph()
    relcount = {'peer-to-peer':0,'provider-to-customer':0}
    while l != "":
        if l.find("#") > -1:
            pass
        else:
            x = l.split("|")
            relcount[arank[x[2]]]+=1
...
def stage1():
    growth  = {}
    mypath = '/home/asemwal/raw_data/scripts/plots/ARank/'
    onlyfiles = [f for f in listdir(mypath) if isfile(join(mypath, f)) and f.find("txt.b") == -1]
    for f in onlyfiles:
        g,relcount= generategraphARank(mypath + f)
        growth.update({f.split(".")[0]:{'p2p': relcount['peer-to-peer'] , 'p2c': relcount['provider-to-customer'],'nodes':g.number_of_nodes(),'edges':g.number_of_edges()}})
    return growth
...
plt.loglog( y1['p2c'],peering['p2c'],'bs' ,label = label[2] )
plt.loglog( y2['p2c'],peering2['p2c'],'rs',label = label[3] )
plt.loglog( y1['c2p'],peering['c2p'],'bx' ,label = label[4] )
