...
def plotccdf(graphfile1='/home/asemwal/raw_data/scripts/plots/ARank/20170901.as-rel2.txt'):
    y=generategraph2(graphfile1)
    x=generategraph2(graphfile1)
    ccdf= {'c2p':{} , 'p2p':{} , 'p2c':{} }
    ccdf2= {'c2p':{} , 'p2p':{} , 'p2c':{} }
    peering= {'c2p':[] , 'p2p':[] , 'p2c':[] }
    y2= {'c2p':[] , 'p2p':[] , 'p2c':[] }
...
def stage1():
    growth  = {}
    mypath = '/home/asemwal/raw_data/experiments/graphs/done/'
    onlyfiles = [f for f in listdir(mypath) if isfile(join(mypath, f)) and f.find("graph_") > -1  and f.find("motif") == -1]
    for f in onlyfiles:
        plotccdf(mypath+f)
