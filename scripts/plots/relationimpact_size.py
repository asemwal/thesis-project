...
file = open('/home/asemwal/raw_data/experiments/results/impact_relations_monsize5'+gname,'r')
...
for i in keys:
    for k in [0,1,2]:
        ...
        plt.ylim((00,35))
        plt.xlim((0,0.9))
        plt.xlabel('Incorrect relationship count (normalized)', fontsize=16)
        ...
        print x
        plt.plot(y,x ,color = color[k], label=label[k], lw=3, alpha=0.8)
        plt.fill_between(y,x1,x2,color = color[k], alpha=0.25)
    plt.legend(loc=0,fontsize=14)
    plt.savefig('relationshipimpact_monsize_'+str(i)+gname+'.pdf', format='pdf', dpi=5000)
    plt.show()
